...
NSArray *PerformXMLXPathQueries(NSData *document, NSArray *queries)
{
	xmlDocPtr doc;
	...
	NSMutableArray *results = [[NSMutableArray alloc] initWithCapacity:[queries count]];
	
	int i = 0;
	for(NSString *str in queries)
	{
		NSArray *result = PerformXPathQuery(doc, str);
